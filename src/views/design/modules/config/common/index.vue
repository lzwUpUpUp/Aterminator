...
<template>
  <div class="common-template">
    <a-collapse defaultActiveKey="0" :bordered="false">
      <a-collapse-panel header="名称" key="0" class="visible-collapse-panel">
        <div class="comment-template__item">
          <p class="comment-template__leading">名称:</p>
          <div class="comment-template__inner">
            <a-input type="text" v-model="config.commonConfig.widgetName" @change="widgetNameChange(config)" />
          </div>
        </div>
      </a-collapse-panel>
      <!-- S 背景颜色 -->
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("screen", ["scale", "widgets"]),
  ...
  methods: {
    ...
    colorModeChange(config) {
      const backgroundColor =
        config.commonConfig.colorMode === "single"
          ? this.singleColor
          : this.linearColor;
    ...
    change(type, trigger = null) {
      switch (type) {
        ...
        case "size":
          ...
          const sizePreConfig = _.cloneDeep(
            this.activeWidget.config.commonConfig
          );
          const sizeMutation = {
            event: {
              distance:
                trigger === "width"
                  ? (this.config.commonConfig.width - sizePreConfig.width) *
                    this.scale
                  : (this.config.commonConfig.height - sizePreConfig.height) *
                    this.scale,
              eventType: "SINGLE",
              mouseType: "mousemove",
        ...
        case "position":
          ...
          const positionPreConfig = _.cloneDeep(
            this.activeWidget.config.commonConfig
          );
          const positionMutation = {
            event: {
              ...
              position: {
                top:
                  (this.config.commonConfig.top - positionPreConfig.top) *
                  this.scale,
                left:
                  (this.config.commonConfig.left - positionPreConfig.left) *
                  this.scale
              },
              type: "move"
            },
    ...
    updateActiveWidget() {
     ...
     widgetNameChange({ commonConfig: { widgetName } }) {
      this.change();
    },
  }
};
