              ...
              <a-collapse-panel header="颜色" key="1" class="visible-collapse-panel">
                <div class="comment-template__item">
                  ...
                  <div class="comment-template__inner">
                    <a-select
                      v-model="config.proprietaryConfig.needleItemStyle.type"
                ...
                <div
                  ...
                  v-if="config.proprietaryConfig.needleItemStyle.type === 'single'"
                >
                  <p class="comment-template__leading">单一颜色:</p>
                  <div class="comment-template__inner">
                    <ColorPicker
                      v-model="config.proprietaryConfig.needleItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.needleItemStyle.type === 'combination'">
                  <div class="comment-template__item">
                    <p class="comment-template__leading">组合类型:</p>
                    <div class="comment-template__inner">
                      <a-select
                        v-model="config.proprietaryConfig.needleItemStyle.colorType"
                        @change="colorGroupChange(config)"
                      >
                  ...
                  <div v-if="config.proprietaryConfig.needleItemStyle.colorType === 'default'">
                    <div class="comment-template__item">
                      <p class="comment-template__leading">配色方案:</p>
                      <div class="comment-template__inner">
                        <a-select
                          v-model="config.proprietaryConfig.needleItemStyle.colorScheme"
                          @change="colorGroupChange(config)"
                        >
                          <a-select-option value="default">默认</a-select-option>
                    ...
                    <div class="comment-template__item needle-config__colors">
                      <span
                        v-for="(color, index)
                          in getColors(config)"
                        :key="index"
                  ...
                  <div v-if="config.proprietaryConfig.needleItemStyle.colorType === 'custom'">
                    <SingleColorSelector
                      v-model="config.proprietaryConfig.needleItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.needleItemStyle.type === 'linear'">
                  <div class="comment-template__item">
                  ...
                  <div v-if="config.proprietaryConfig.needleItemStyle.colorType === 'default'">
                    ...
                    <div class="comment-template__item needle-config__linear">
                      <span
                        v-for="(color, index)
                          in getColors(config)"
                        :key="index"
...
export default {
  ...
  methods: {
    ...
    getColors(config) {
      ...
      if (config.proprietaryConfig.needleItemStyle.type === "single") {
        return this.singleColor;
      }

      let color;
      if (config.proprietaryConfig.needleItemStyle.colorType === "custom") {
        color = [
          ...this[
            customMapping.get(config.proprietaryConfig.needleItemStyle.type)
          ]
        ];
      } else {
        const { colors } = this[
          typeMapping.get(config.proprietaryConfig.needleItemStyle.type)
        ].find(
          item =>
            item.type === config.proprietaryConfig.needleItemStyle.colorScheme
        );
        color = [...colors];
      }
      return color;
    ...
    singleColorChange(config) {
      this.singleColor = config.proprietaryConfig.needleItemStyle.color;
      this.change();
    },
    /**
     * 组合颜色选择
    ...
    combinationColorChange(config) {
      this.combinationCustomColors =
        config.proprietaryConfig.needleItemStyle.color;
      this.change();
    },
    /**
     * 渐变颜色选择
    ...
    linearColorChange(config) {
      this.linearCustomColors = config.proprietaryConfig.needleItemStyle.color;
      this.change();
    },
    /**
     ...
     */
    colorChange(config) {
      Object.assign(config.proprietaryConfig.needleItemStyle, {
