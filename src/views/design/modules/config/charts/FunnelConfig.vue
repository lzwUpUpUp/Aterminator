              ...
              <a-collapse-panel header="颜色" key="1" class="visible-collapse-panel">
                <div class="comment-template__item">
                  ...
                  <div class="comment-template__inner">
                    <a-select
                      v-model="config.proprietaryConfig.funnelItemStyle.type"
                ...
                <div
                  ...
                  v-if="config.proprietaryConfig.funnelItemStyle.type === 'single'"
                >
                  <p class="comment-template__leading">单一颜色:</p>
                  <div class="comment-template__inner">
                    <ColorPicker
                      v-model="config.proprietaryConfig.funnelItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.funnelItemStyle.type === 'combination'">
                  <div class="comment-template__item">
                    <p class="comment-template__leading">组合类型:</p>
                    <div class="comment-template__inner">
                      <a-select
                        v-model="config.proprietaryConfig.funnelItemStyle.colorType"
                        @change="colorGroupChange(config)"
                      >
                  ...
                  <div v-if="config.proprietaryConfig.funnelItemStyle.colorType === 'default'">
                    <div class="comment-template__item">
                      <p class="comment-template__leading">配色方案:</p>
                      <div class="comment-template__inner">
                        <a-select
                          v-model="config.proprietaryConfig.funnelItemStyle.colorScheme"
                          @change="colorGroupChange(config)"
                        >
                          <a-select-option value="default">默认</a-select-option>
                  ...
                  <div v-if="config.proprietaryConfig.funnelItemStyle.colorType === 'custom'">
                    <SingleColorSelector
                      v-model="config.proprietaryConfig.funnelItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.funnelItemStyle.type === 'linear'">
                  <div class="comment-template__item">
...
export default {
  ...
  methods: {
    ...
    getColors(config) {
      ...
      if (config.proprietaryConfig.funnelItemStyle.type === "single") {
        return this.singleColor;
      }

      let color;
      if (config.proprietaryConfig.funnelItemStyle.colorType === "custom") {
        color = [
          ...this[
            customMapping.get(config.proprietaryConfig.funnelItemStyle.type)
          ]
        ];
      } else {
        const { colors } = this[
          typeMapping.get(config.proprietaryConfig.funnelItemStyle.type)
        ].find(
          item =>
            item.type === config.proprietaryConfig.funnelItemStyle.colorScheme
        );
        color = [...colors];
      }
      return color;
    ...
    singleColorChange(config) {
      this.singleColor = config.proprietaryConfig.funnelItemStyle.color;
      this.change();
    },
    /**
     * 组合颜色选择
    ...
    combinationColorChange(config) {
      this.combinationCustomColors =
        config.proprietaryConfig.funnelItemStyle.color;
      this.change();
    },
    /**
     * 渐变颜色选择
    ...
    linearColorChange(config) {
      this.linearCustomColors = config.proprietaryConfig.funnelItemStyle.color;
      this.change();
    },
    /**
     ...
     */
    colorChange(config) {
      Object.assign(config.proprietaryConfig.funnelItemStyle, {
