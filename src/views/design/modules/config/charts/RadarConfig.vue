        ...
        <ChartProprietaryTemplate>
          <template v-slot:header>
            <a-collapse :bordered="false">
              <a-collapse-panel header="雷达类型" key="1" class="visible-collapse-panel">
                <div class="comment-template__item">
                  <p class="comment-template__leading">类型:</p>
                  <div class="comment-template__inner">
                    <a-select v-model="config.proprietaryConfig.radar.shape" @change="change">
                      <a-select-option value>多边形</a-select-option>
              ...
              <a-collapse-panel header="颜色" key="2" class="visible-collapse-panel">
                <div class="comment-template__item">
                  <p class="comment-template__leading">颜色类型:</p>
                  <div class="comment-template__inner">
                    <a-select
                      v-model="config.proprietaryConfig.radarItemStyle.type"
                ...
                <div
                  ...
                  v-if="config.proprietaryConfig.radarItemStyle.type === 'single'"
                >
                  <p class="comment-template__leading">单一颜色:</p>
                  <div class="comment-template__inner">
                    <ColorPicker
                      v-model="config.proprietaryConfig.radarItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.radarItemStyle.type === 'combination'">
                  <div class="comment-template__item">
                    <p class="comment-template__leading">组合类型:</p>
                    <div class="comment-template__inner">
                      <a-select
                        v-model="config.proprietaryConfig.radarItemStyle.colorType"
                        @change="colorGroupChange(config)"
                      >
                  ...
                  <div v-if="config.proprietaryConfig.radarItemStyle.colorType === 'default'">
                    <div class="comment-template__item">
                      <p class="comment-template__leading">配色方案:</p>
                      <div class="comment-template__inner">
                        <a-select
                          v-model="config.proprietaryConfig.radarItemStyle.colorScheme"
                          @change="colorGroupChange(config)"
                        >
                          <a-select-option value="default">默认</a-select-option>
                    ...
                    <div class="comment-template__item funnel-config__colors">
                      <span
                        v-for="(color, index)
                          in getColors(config)"
                        :key="index"
                  ...
                  <div v-if="config.proprietaryConfig.radarItemStyle.colorType === 'custom'">
                    <SingleColorSelector
                      v-model="config.proprietaryConfig.radarItemStyle.color"
                ...
                <div v-if="config.proprietaryConfig.radarItemStyle.type === 'linear'">
                  <div class="comment-template__item">
                  ...
                  <div v-if="config.proprietaryConfig.radarItemStyle.colorType === 'default'">
                    ...
                    <div class="comment-template__item funnel-config__linear">
                      <span
                        v-for="(color, index)
                          in getColors(config)"
                        :key="index"
...
import chartLinkpage from "../chartLinkpage/chartLinkpage";
...
export default {
  ...
  methods: {
    ...
    getColors(config) {
      ...
      if (config.proprietaryConfig.radarItemStyle.type === "single") {
        return this.singleColor;
      }

      let color;
      if (config.proprietaryConfig.radarItemStyle.colorType === "custom") {
        color = [
          ...this[
            customMapping.get(config.proprietaryConfig.radarItemStyle.type)
          ]
        ];
      } else {
        const { colors } = this[
          typeMapping.get(config.proprietaryConfig.radarItemStyle.type)
        ].find(
          item =>
            item.type === config.proprietaryConfig.radarItemStyle.colorScheme
        );
        color = [...colors];
      }
      return color;
    ...
    singleColorChange(config) {
      this.singleColor = config.proprietaryConfig.radarItemStyle.color;
      this.change();
    },
    /**
     * 组合颜色选择
    ...
    combinationColorChange(config) {
      this.combinationCustomColors =
        config.proprietaryConfig.radarItemStyle.color;
      this.change();
    },
    /**
     * 渐变颜色选择
    ...
    linearColorChange(config) {
      this.linearCustomColors = config.proprietaryConfig.radarItemStyle.color;
      this.change();
    },
    /**
     ...
     */
    colorChange(config) {
      Object.assign(config.proprietaryConfig.radarItemStyle, {
