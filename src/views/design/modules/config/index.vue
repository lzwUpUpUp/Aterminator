...
export default {
  ...
  computed: {
    ...
    templateName() {
      return TEMPLATES.find(
        template => template.type === this.activeWidget.config.type
      ).name;
    },
    // 使用公共标题
    hasCommonTitle() {
  ...
  methods: {
    ...
    dataSet() {
      //初始化时获取数据集
      this.$axios({
        url: "/dashboard/dataSet/treeData",
        type: "get"
      }).then(res => {
        // console.log('res',res);
        localStorage.setItem("dsTree", JSON.stringify(res.data.treeData));
      });
    }
  }
};
