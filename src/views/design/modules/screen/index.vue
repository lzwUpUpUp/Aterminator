  ...
  <div class="screen">
    <div class="screen__header">
      ...
      <div class="screen__bar">
        <div class="screen__front"></div>
        <div class="screen__back">
          <a-button-group>
            <a-popconfirm title="确定清空画板？" @confirm="clear" okText="确定" cancelText="取消">
              <a-tooltip>
    ...
    <!-- S 画板 -->
    <div ref="page" class="page" @click.self="() => select$.next({ el: 'view' })">
    ...
    <div class="scale-bar">
      <a-tooltip placement="top">
        <template slot="title">自动比例：{{ isAutoResize ? '已开启' : '已关闭' }}</template>
        <a-switch
          size="small"
...
export default {
  ...
  data: () => ({
    ...
    // 承接来自design页面信息
    viewData: "",
    resourseId:'', //编辑页面的页面ID
  }),
  watch: {
    message(newVal, oldVal) {
      this.viewData = newVal;
      this.resourseId = this.viewData.resourseId
      this.updateConfig();
    }
  },
  mounted() {
    ...
    merge(
      ...
      zip(
        ...
        filter(({ events: [mousedownEvent] }) =>
          [...mousedownEvent.target.classList].includes("view")
        )
      ),
      this.wrapperChange$
    )
    ...
    this.adjust$
      ...
      .subscribe(mutation => {
        ...
        const {
          event: { mouseType, eventType }
        } = mutation;
        // 当鼠标抬起时更新部件位置状态
        if (mouseType === "mouseup") {
          const { top, left, width, height } = window.getComputedStyle(
            targetComponent.$el,
            null
          );
      ...
      .pipe(
        ...
        tap(([down, move]) => {
          ...
          anime.set(this.$refs.selector.$el, {
            display: "block",
            translateX:
              movePageX - downPageX > 0
                ? translateX
                : translateX + (movePageX - downPageX),
            translateY:
              movePageY - downPageY > 0
                ? translateY
                : translateY + (movePageY - downPageY),
  ...
  methods: {
    ...
    save() {
      ...
      let daResource = {
        id:this.resourseId,
        name: this.viewOptions.config.commonConfig.name, // 资源名称
        type: 2, //资源类型,值为2固定为2
        categoryId: this.viewOptions.config.commonConfig.directoryId, //资源目录ID
        categoryName: this.$store.state.directoryName //资源目录名称
      };
      console.log("daResource", daResource);
      //保存页面
      this.$axios({
        url: "/dashboard/report/design/save",
        method: "post",
        headers: {
          "content-type": "application/x-www-form-urlencoded;chartset=utf-8"
        },
        data: Qs.stringify({
          daResource: encodeURIComponent(JSON.stringify(daResource)),
          daPage: encodeURIComponent(JSON.stringify(this.viewOptions))
        })
      }).then(res => {
        if (res.message == "成功") {
    ...
    updateConfig() {
      this.setView({
        view: new View({
          ...this.view,
          config: {
            commonConfig: this.viewData.config.commonConfig
          }
        })
      });

      const widgets = this.viewData.widgets.map(
        config => new WidgetModel(config)
      );
      // 更新视图对象
      this.setView({
        view: new View({
    ...
    importConfig(file) {
      ...
      reader.onload = () => {
        ...
        // 实例化部件对象
        const widgets = this.viewOptions.widgets.map(
          config => new WidgetModel(config)
        );
        // 更新视图对象
        this.setView({
